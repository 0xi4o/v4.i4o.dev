...
This site has no test suite and does not want one — it is prose and static pages, validated by
`vp check` and a full `vp run build`. Don't add tests, a `test` block, or `vp test` to any
checklist. (The Vite+ block above is regenerated by `vp config`; if it comes back mentioning
`vp test`, this section wins.)
